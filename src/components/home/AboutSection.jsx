...
const AboutSection = ({
  ...
  imageFit = 'cover',
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/set-state-in-effect -- reset load status when paper deps change
        setStatus('loading');
        hit.disabled = true;
    ...
    const cappedScale = maxDisplayWidth > 0 ? Math.min(initialScale, maxDisplayWidth / initialWidth) : initialScale;
    root.style.setProperty('--pc-image-width', `${initialWidth * cappedScale}px`);
    root.style.setProperty('--pc-image-height', `${initialHeight * cappedScale}px`);
        let renderer;
        try {
    ...
    function resize() {
      ...
      if (maxDisplayWidth > 0) scale = Math.min(scale, maxDisplayWidth);
        ...
        if (src) {
            ...
            setStatus('error');
            options.current.onError?.(new Error('AboutSection requires an image src.'));
        }
        return () => {
            disposed = true;
    ...
    return (
        <section
            ref={rootRef}
            className={`relative isolate flex w-full items-center justify-center overflow-hidden ${className}`}
        ...
        >
            ...
            {status !== 'ready' && (
                <img
                    className="pointer-events-none absolute left-1/2 top-1/2 h-(--pc-image-height,72%) w-(--pc-image-width,60%) select-none"
                    src={src || undefined}
                    alt={alt}
                    draggable={false}
                    style={{
            ...
            <button
                ref={hitRef}
                type="button"
                className="peer absolute left-0 top-0 m-0 cursor-grab touch-none appearance-none select-none rounded-xs border-0 bg-transparent p-0 shadow-none! outline-none! [-webkit-tap-highlight-color:transparent] data-[held=true]:cursor-grabbing focus:outline-none! focus-visible:outline-none! disabled:cursor-default"
